...
import sokol.gfx
import src.textures
import src.transform { Vector2, Vector3, Vertex }

#flag -I @VMODROOT/.
#include "shaders/block_glsl.h"
// This is generated with `v shader ./shaders`
...
fn init(mut game Game) {
	...
	{ // initialize glsl pipeline
		mut vertex_buffer_desc := gfx.BufferDesc{
			label: c'block-vertices'
		}
		unsafe { vmemset(&vertex_buffer_desc, 0, int(sizeof(vertex_buffer_desc))) }
		sizeof_vertex_struct := int(sizeof(Vertex))
		vertex_buffer_desc.size = usize(block_vertices.len * sizeof_vertex_struct)
		vertex_buffer_desc.data = gfx.Range{
			ptr: block_vertices.data
			size: usize(block_vertices.len * sizeof_vertex_struct)
		}
		vertex_buffer_desc.@type = .vertexbuffer
		// vertex_buffer_desc.usage = .immutable
		vbuf := gfx.make_buffer(&vertex_buffer_desc)

		mut index_buffer_desc := gfx.BufferDesc{
			label: c'block-indices'
		}
		unsafe { vmemset(&index_buffer_desc, 0, int(sizeof(index_buffer_desc))) }
		sizeof_byte := int(sizeof(u8))
		index_buffer_desc.size = usize(block_indices.len * sizeof_byte)
		index_buffer_desc.data = gfx.Range{
			ptr: block_indices.data
			size: usize(block_indices.len * sizeof_byte)
		}
		index_buffer_desc.@type = .indexbuffer
		ibuf := gfx.make_buffer(&index_buffer_desc)

		// shader and glsl pipeline
		shader := gfx.make_shader(C.cube_shader_desc(C.sg_query_backend()))

		mut pline_desc := gfx.PipelineDesc{}
		unsafe { vmemset(&pline_desc, 0, int(sizeof(pline_desc))) }
		pline_desc.layout.buffers[0].stride = sizeof_vertex_struct
		// the constants [C.ATTR_vs_pos, C.ATTR_vs_color0, C.ATTR_vs_texcoord0] are generated bysokol-shdc
		pline_desc.layout.attrs[C.ATTR_vs_pos].format = .float3			// x,y,z as f32
		pline_desc.layout.attrs[C.ATTR_vs_color0].format = .ubyte4n		// color as u32
		pline_desc.layout.attrs[C.ATTR_vs_texcoord0].format = .float2	// u,v as f32

		pline_desc.shader = shader
		pline_desc.index_type = .uint16
		pline_desc.depth = gfx.DepthState{
			write_enabled: true
			compare: .less_equal
		}
		pline_desc.cull_mode = .back
		pline_desc.label = 'glsl_shader pipeline'.str
		game.bindings.vertex_buffers[0] = vbuf
		game.bindings.index_buffer = ibuf
		game.bindings.fs_images[C.SLOT_tex] = textures.bytes_to_gfximage(textures.block_test_bytes, 'block_test')
		game.glsl_pipeline = gfx.make_pipeline(&pline_desc)
	}
